from app import app
from flask import Flask, render_template, request, flash, session, redirect, url_for  # noqa
    ...
    if request.method == 'GET':
        ...
        if form.validate() == True:
            if User.query.filter_by(email=form.email.data).first():
                ...
                newuser = User(form.firstname.data, form.lastname.data, form.email.data, form.password.data)  # noqa
            ...
            for field, errors in form.errors.items():
                for error in errors:
                    log.info(u"Error in the %s field - %s" % (getattr(form, field).label.text, error))  # noqa
            return render_template('register.html', form=form)
...
def doc_sum_project():
    return render_template('doc_summary.html')
...
def summary():
    if request.method == 'POST':
        ...
        if text_input == "url":
            ...
            log.info("File summary selected")
            ...
            if this_file and allowed_file(this_file.filename):
                ...
                this_file.save(os.path.join(UPLOAD_FOLDER, filename))  # noqa
