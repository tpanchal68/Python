...
# **Answer: ** 
# 
# Let's first understand "What is Machine Learning?"  Machine Learning is the science of getting computers to learn and act like humans do.  Machine Learning also helps improve computers their learning over time by feeding them data and information.  Machine Learning algorithms to parse data, learn from it, and then make a prediction about some real life scenarios.  Machine Learning has many different types of algorithms and hew ones are being defined and added on regular basis.  Algorithms are grouped into three categories: supervised learning, unsupervised learning, semi-supervised learning.  Each categories contains various different algorithms based on different mathematical formulas.  For CharityML, I have chosen Gradient Boosting Classifier to predict the donors.  Gradient Boosting Classifier is one of the supervised learning methods.  In supervised learning problems, we use the training data with multiple features X to predict a target variable y. It is considered to be an ensemble method type of model. Ensemble methods basically gathers the results of multiple trained models to create a much better model.  In a simple term, let's say you have a data with five different categories: capital-gain, capital-loss, age, marital-status.  To find out the most probable donor from this categories, first you would split them if they they have capital gain or capital loss.  If they have capital gain, there is high probability that they will donate versus the group of people with capital loss.  Then within the high probability group, you would break down the data further into age group.  Generally speaking, older person may be more likely to donate versus younger; thus, put elderly group into high probability group.  Now if they are married, they may have two source of income and they may have influence from their spouce to charitable cause; thus move them into higher probability group.  Thus as you can see, we continue to build the tree based on the hypothesis and the data.  Gradient Boosting Classifier combines weak learners into a stronger learner in an iterative fashion. It first needs to find misclassification and then “boost” those incorrectly predicted data points. Continue to repeat the process until you can improve the final output of the model. Training of the model stops once loss no longer improves on validation dataset.
# 
# For finding donors project, those weak learners are "n_estimators".  The size of the tree can be controlled by "max_depth".  By using multiple "n_estimators" as weak learners and combining that with "max_depth" as you will see in Model Tuning code, we are able to improve the accuracy of the model. Gradient Boosting Classifier model takes various attributes as inputs to create classifier.  To create the initial model, along with default GBM parameters, we would pass features training set, income training set, features testing set, and income testing set along with the size of samples to be drawn from training set.  GBM processes all of these parameters and returns accuracy and f-score for both training and testing sets for the given sample size.  Since these accuracy and f-score are processed with GBM default parameters, there is a possibility to improve the accuracy and f-score numbers.  To do so, we would use a model tuning technique called grid search.  To create a grid search object, we would pass the classifier, parameter dictionary that we want to fine tune the model to, and fbeta_score scoring object to grid search.  GridSearchCV function will process each parameters and creates the grid search object.  We then pass the features training set and income training set to find the optimal parameters using fit and get the estimator.  Now using estimator, we can make predictions with optimized model.
# 
# 
# **Citations: **
